...
RUN apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates tzdata libasan8 wget python3 \
...
EXPOSE 3000
WORKDIR /data
COPY scripts/configure_responses_channel.py /app/configure_responses_channel.py
COPY scripts/responses_entrypoint.sh /responses-entrypoint.sh
RUN chmod +x /responses-entrypoint.sh
ENTRYPOINT ["/responses-entrypoint.sh"]
